...
export default class AzureInstance {
  ...
  private authorize_endpoint: string;
  ...
  updateToken() {
    if ("refreshToken" in this.token) {
      this.auth!.refreshToken(this.token.refreshToken).then((token) => {
        this.setToken(token as Itoken);
      })
    }
  }
  ...
  loadDataFromStorage() {
    return AsyncStorage.getItem('authToken').then((tokenStr) =>{
      console.log('token', tokenStr)
      if(tokenStr) {
        const token = JSON.parse(tokenStr);
        if (token.expires_in * 1000 > new Date().getTime()) {
  ...
  getUserInfo() {
    ...
    return fetch("https://webservices.scientificnet.org/rest/cm/core/api/UserData/GetUserData/?domain=unibz&username=" + this.userData.username, {
      headers: {
        Authorization: "Bearer " + (this.getToken() as Itoken).accessToken
      },
    })
      .then((response) => {
